...
Salidas reales de consola, sin editar, capturadas con `tee` el 24/09/2026 (`03`, `04`, `05`, `07` y `08` actualizadas el 25/09/2026)
...
| [`05_tests_pytest.txt`](05_tests_pytest.txt) | `pytest -v` | 44 tests que no llaman a ninguna API (usan fakes). |
...
| [`08_consulta_json.txt`](08_consulta_json.txt) | `python -m src.rag_system "¿Cómo agrego CORSMiddleware con allow_origins?" --json` | La consulta de `03` como JSON: `pregunta`, `estrategia_recuperacion`, `indice`, `namespace`, `top_k` y `fragmentos_recuperados` con `chunk_id`, `fuente`, `categoria`, `page`, `score_combinado` y `extracto`. |
